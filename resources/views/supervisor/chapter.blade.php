...
<div class="container submition-container">
    @if (isset($chapters->submition))
        <div class="row submition-background">
            ...
            <div class="col-md-4 m-auto text-end submition-date">date {{ $chapters->submition_date }}</div>
        </div>
    @else
        <div class="row">
    ...
    <div class="row p-3">
        ...
        <div class="row ps-4 ">
            <div class="col text-start submition-deadline">{{ $chapters->deadline }}</div>
...
<form
    action="{{ route('progressinput', ['chapter_id' => $chapters->id]) }}"
            ...
            <div class="container container-progress-input pt-3">
                <div class="progress progress-bar-input" role="progressbar" aria-label="Example with label"
                    ...
                    <div class="progress-bar main-progress-bar-input-scale" id="progressBar"
                        style="width: {{ $chapters->chapter_status }}%">
                        <value class="main-progress-bar-input-value" id="progressValue">
                            {{ $chapters->chapter_status }}%</value>
                    </div>
                </div>
                <div class="slider-container">
                    <input name="progress_input" type="range" class="slider" id="mySlider"
...
<div class="container m-auto align-middle student-statement-container">
    ...
    <div class="statement-text-container p-4 pt-3 container text-start">
        ...
        <div class="statment-edit-container p-0 m-0" id="statment-edit-container">
            <form method="POST"
                action="{{ route('UpdateStatement', ['chapter_id' => $chapters->id]) }}"
                class="p-0 m-0" id="update-text-form">
                @csrf
                {{ csrf_field() }}
                <textarea v-model="content" id="tinymcestatement" class="tinymce-editor "
                    name="supervisor_statement">{{ $chapters->supervisor_statement }}</textarea>
...
<div class="container ">
    ...
    <div class="container history_files_container p-4 d-none" id="history_files_container">
        ...
        @foreach($chapters->chapter_history as $chapter_history)

            <div class="row history_file_row mb-3 mt-3">
                <div class="col-md-1 text-start ">
                    <a href="/public/chapters_tasks/{{ $chapter_history->file_adress }}" download>
                        <i class="bi bi-file-earmark-text-fill history_file_icon"></i>
                    </a>
                </div>
                <div class="col-md-7 m-auto ps-0 text-start file_history_text">{{ $chapter_history->file_adress }}</div>
                <div class="col-md-4 m-auto text-end ">{{ $chapter_history->created_at }}</div>
            </div>
...
<footer class="pb-5 pt-5">
    <div class="container">
        <div class="row justify-content-evenly text-start">
            @if (!empty($chapters->guidline))
                <div class="col-3 guidline_container">
                    <a class="p-0" href="/public/chapters_tasks/{{ $chapters->guidline }}" download>
                        <i class="bi bi-file-earmark-text-fill submition-icon"></i> Guideline
                    </a>
                </div>
            @endif

            @if (!empty($chapters->template))
                <div class="col-3 template_container">
                    <a class="p-0" href="/public/chapters_tasks/{{ $chapters->template }}" download>
